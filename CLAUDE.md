...
- **Installable** in Chrome and Edge as a standalone app via `vite-plugin-pwa`.
- **Manifest**: configured in `client/vite.config.js` with app name, icons (192/512 standard + maskable), standalone display, and theme/background colors.
- **Service Worker**: Workbox-generated with `autoUpdate`. Caches static assets; excludes `/ws` and `/api` routes.
- **Dev mode**: `devOptions.enabled: true` is required for the install prompt to appear during `vite dev`. Without it, manifest and SW are only injected in production builds.
- **Icons**: source SVG + pre-generated PNGs in `client/public/`. Maskable variants have extra padding for Android adaptive icon safe zones.
- **Apple meta tags**: `apple-mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style`, and `apple-touch-icon` are in `client/index.html`.
...
- The `dev-dist/` directory is generated by vite-plugin-pwa in dev mode and is gitignored.
