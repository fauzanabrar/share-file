...
The app is installable as a Progressive Web App in Chrome and Edge. `vite-plugin-pwa` is configured in `client/vite.config.js` with:
- **Manifest**: name, short_name, icons (192/512 standard + maskable), standalone display, theme/background colors.
- **Service Worker**: Workbox-generated `sw.js` with `autoUpdate` registration. Caches static assets (JS, CSS, HTML, PNG, SVG). WebSocket (`/ws`) and API (`/api`) routes are excluded from caching.
- **Dev mode**: `devOptions.enabled: true` ensures the manifest and service worker are injected during `vite dev`, not just in production builds. Without this, the install prompt will not appear in development.
- **Icons**: source SVG in `client/public/icon.svg`, pre-generated PNGs (192/512 standard + maskable) also in `public/`. The `dev-dist/` directory is auto-generated by vite-plugin-pwa in dev mode and should be gitignored.
- **Apple PWA meta tags**: `apple-mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style` (black-translucent), and `apple-touch-icon` are in `client/index.html`.
...
- `vite-plugin-pwa` requires `devOptions: { enabled: true }` to inject the manifest and service worker in dev mode. Without it, the browser install prompt will not appear during development.
- The `dev-dist/` directory is generated by vite-plugin-pwa in dev mode. It is gitignored and should not be committed.
- PWA icons include both standard and maskable variants. Maskable icons have extra padding for Android adaptive icon safe zones.
