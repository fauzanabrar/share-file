...
- **Auto-Retry & Relay Fallback**: Automatically schedules WebRTC re-negotiations if a channel fails. If P2P connection is impossible (e.g., VPS deployments across different networks), it seamlessly falls back to relaying data through the WebSocket server with automatic backpressure support.
- **Dynamic Network Warnings**: Provides a clean, dismissible UI tip encouraging users to use the same Wi-Fi network for optimal speed, and subtly displays an amber "⚠️ Relay" badge on devices connected via the fallback.
- **Installable PWA**: Full Progressive Web App support with service worker caching, web manifest, and standalone display mode. Install from Chrome or Edge for a native app-like experience with offline app shell caching.
...
8. If WebRTC negotiation fails entirely, the connection gracefully switches to a WebSocket Relay mode to ensure file delivery succeeds over the internet.
...
- `VITE_TURN_URL` - TURN server URL (e.g. `turn:1.2.3.4:3478`). Optional for internet/VPS deployment, as the app will automatically fall back to the WebSocket relay if direct connection fails.
